...
}var u=v.split("/");var w=x.substring(0,x.lastIndexOf("/")).split("/");w=w.length>0?w:[];d(u,function(){if(this==".."){w.pop();}else{if(this=="."){}else{w.push(this);
}}});return w.join("/");};var i=function(w,v){if((typeof w)=="string"){w=[w];}var u=[];d(w,function(){var x=p[this]||this;u.push(h(x,v));});return u;};
var m=function(u){var u=this.uri=this.id=u||"/";this.resovleUri=function(v){return h(v,u);};this.require=function(x,y){var w=i(x,u);var v=a.load(w,y);return v&&v.length==1?v[0]:v;
};this.exports={};this.declare=null;this.deps=null;this.loaded=false;};var l=[];a.define=function(z,y,v){var w=null;if(y&&v){w={moduleDeps:y,moduleDeclare:v};
}else{if(z&&y){w={moduleDeps:z,moduleDeclare:y};}else{if(z&&v){w={moduleDeps:y,moduleDeclare:v};}else{if(z){w={moduleDeclare:z};}}}}if(w){if(typeof w.moduleDeclare!="function"){var A=w.moduleDeclare;
